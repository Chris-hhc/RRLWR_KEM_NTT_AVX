...
static int test_commutativity() {
  ring_element a, s, sp, b, bp, r1, r2;
  unsigned char pb[RRLWR_K*RRLWR_PKE_PACKED_POLYP_LEN], pbp[RRLWR_K*RRLWR_PKE_PACKED_POLYP_LEN];
  unsigned char ps[RRLWR_K*RRLWR_PACKED_POLY2_LEN], psp[RRLWR_K*RRLWR_PACKED_POLY2_LEN];
  ...
  ring_uniform(&s, RRLWR_PKE_LOG_ETA+1, seedS, RRLWR_SEED_S_LEN);
  ring_uniform(&sp, RRLWR_PKE_LOG_ETA+1, seedSp, RRLWR_SEED_S_LEN);
  ring_mul32(r1.x, &bp, &s, RRLWR_K, RRLWR_PKE_PRIME, RRLWR_PKE_PRIMEINV, RRLWR_NTTINV_FINALCONST, RRLWR_KEM_RMODPRIME, RRLWR_KEM_2RMODPRIME, rrlwr_pke_zetas);
  ring_mul32(r2.x, &b, &sp, RRLWR_K, RRLWR_PKE_PRIME, RRLWR_PKE_PRIMEINV, RRLWR_NTTINV_FINALCONST, RRLWR_KEM_RMODPRIME, RRLWR_KEM_2RMODPRIME, rrlwr_pke_zetas);

  assert(!memcmp(&r1, &r2, RRLWR_K*(RRLWR_N << 2)));

  // Test with rounding
  ring_uniform(&a, RRLWR_PKE_LOGQ, seedA, RRLWR_PKE_SEED_A_LEN);
  ring_uniform(&s, RRLWR_PKE_LOG_ETA+1, seedS, RRLWR_SEED_S_LEN);
  ring_pack(ps, &s, RRLWR_PKE_LOG_ETA+1);
  ring_mul32(b.x, &a, &s, RRLWR_K, RRLWR_PKE_PRIME, RRLWR_PKE_PRIMEINV, RRLWR_NTTINV_FINALCONST, RRLWR_KEM_RMODPRIME, RRLWR_KEM_2RMODPRIME, rrlwr_pke_zetas);
  ring_round_xtoy(&b, &b, RRLWR_PKE_LOGQ, RRLWR_PKE_LOGP);
  ring_pack(pb, &b, RRLWR_PKE_LOGP);
  ring_unpack(&b, pb, RRLWR_PKE_LOGP);
  ring_uniform(&a, RRLWR_PKE_LOGQ, seedA, RRLWR_PKE_SEED_A_LEN);
  ring_uniform(&sp, RRLWR_PKE_LOG_ETA+1, seedSp, RRLWR_SEED_S_LEN);
  ring_pack(psp, &sp, RRLWR_PKE_LOG_ETA+1);
  ring_mul32(bp.x, &a, &sp, RRLWR_K, RRLWR_PKE_PRIME, RRLWR_PKE_PRIMEINV, RRLWR_NTTINV_FINALCONST, RRLWR_KEM_RMODPRIME, RRLWR_KEM_2RMODPRIME, rrlwr_pke_zetas);
  ring_round_xtoy(&bp, &bp, RRLWR_PKE_LOGQ, RRLWR_PKE_LOGP);
  ring_pack(pbp, &bp, RRLWR_PKE_LOGP);
  ring_unpack(&bp, pbp, RRLWR_PKE_LOGP);
  ring_unpack(&s, ps, RRLWR_PKE_LOG_ETA+1);
  ring_unpack(&sp, psp, RRLWR_PKE_LOG_ETA+1);
  ring_mul32(r1.x, &bp, &s, RRLWR_K, RRLWR_PKE_PRIME, RRLWR_PKE_PRIMEINV, RRLWR_NTTINV_FINALCONST, RRLWR_KEM_RMODPRIME, RRLWR_KEM_2RMODPRIME, rrlwr_pke_zetas);
  ring_mul32(r2.x, &b, &sp, RRLWR_K, RRLWR_PKE_PRIME, RRLWR_PKE_PRIMEINV, RRLWR_NTTINV_FINALCONST, RRLWR_KEM_RMODPRIME, RRLWR_KEM_2RMODPRIME, rrlwr_pke_zetas);
  ring_round_xtoy(&r1, &r1, RRLWR_PKE_LOGP, RRLWR_PKE_LOGT);
  ring_round_xtoy(&r2, &r2, RRLWR_PKE_LOGP, RRLWR_PKE_LOGT);
  ring_round_xtoy(&r1, &r1, RRLWR_PKE_LOGT, 1);
  ring_round_xtoy(&r2, &r2, RRLWR_PKE_LOGT, 1);

  // Add an assertion that this is small...
  //assert(!memcmp(&r1, &r2, RRLWR_K*(RRLWR_N << 2)));
